...
**Status**: built & verified (2026-08-25, click-tested by the user — both tabs, copy, import round-trip OK)
...
**MVP scope**
- **A. AI prompts — 2 new tabs** in the existing ✦ AI prompt modal (4 tabs total): **Personas** (role → short grill-style interview: segments, goals, research to mine → persona field semantics → `kind: 'personas'` contract + example → single ```json block) and **Brand themes** (role → derive brand colors from a given brand/site/guidelines or mood, ask one short round of questions if nothing is given → the 13-role color table with what each role paints → WCAG AA guidance for text roles → `kind: 'themes'` contract + example → single ```json block). Both self-contained English constants, always available (no journey needed). Import via the existing ↥ (both branches already exist).
- **B. Persona editor modal** — all fields editable in the UI: `Name`, `Role`, `Description`, `Quote`, `Goals` / `Needs` / `Frustrations` (textareas, **one item per line**). Entry points: `+` in the Personas header (create), ✎ on a persona item (edit), **Edit button on the persona card**. Save normalizes lists, auto-names empty (`Persona N`), refreshes sidebar + journey-meta + journey list.
- **C. Theme editor modal** — `Name` + a row per **theme role** (13 roles, friendly label + what it paints): color swatch (`<input type="color">`) ↔ hex text input ↔ clear × (empty hex = role unset → app-token fallback). A **live preview strip** (stage band, step header, slice label, cell with a touchpoint pill + text) recolors as you type. Save: create (`+` in the Themes header — new button) or edit (✎ on a theme item); colors are **replaced wholesale** on save (the editor is the full picture, unlike import's per-key merge). Editing a theme in use re-themes the grid immediately.
- **D. Folded fix — personas import tolerance**: the personas branch of `importFromFile()` also accepts `{personas: [...]}` **without** `kind` (mirrors Change 9 for themes) — an AI answer with the wrapper key but no `kind` must not be rejected. Bare arrays stay unclaimed (a bare `{name,…}` object array is indistinguishable from themes).

**Later (deferred)**
- Dictionary prompts (touchpoints / follow-up channels) as further tabs (already Change 15 Later).
- "Expand personas" prompt variant (AI round-trip over the existing persona pool, like Expand journey).
- Theme duplicate button; WCAG contrast warnings inside the editors; eyedropper from a pasted logo/screenshot.
- Duplicate-name warnings in the editors (persona/theme import merges by name).
- Persona avatar/photo (already Change 7 Later).

**Impact**
- **Data**: **none new** — persona fields exist since Change 7, `brandThemes` since Change 8; editors write the existing shapes. **No key version bump**, no storage changes.
- **Actions**: + `openPersonaEditor(personaId|null)`, `openThemeEditor(themeId|null)`; `createPersona()` removed (the `+` opens the editor instead — its `_pendingRename = 'persona'` flag was already dead, `renderPersonaList` never consumed it); `mkSidebarItem` gains an optional `onEdit` (✎ opens a modal instead of inline rename — dblclick inline rename stays bound to `onRename`); `openPersonaCard` gains an Edit footer button; `openAiPrompt` refactored to a tab-descriptor list; personas import detection loosened (D).
- **Screens**: AI prompt modal → 4 tabs (relabel: *Create journey · Expand journey · Personas · Brand themes*); persona editor modal (persona-card pattern); theme editor modal (tp-manager pattern, wider); `+` in the Themes section header; ✎ on persona and theme items; Edit button on the persona card; refreshed empty-state copy (Personas list, Themes list, persona card).
- **States**: editor with empty name → auto-name on save (`Persona N` / `Theme N`); invalid hex in a role's text field → that role is simply not saved (and previewed as fallback), the field is not cleared; empty lists → `[]`; theme with every role cleared → legal (all fallbacks); no journey open → theme editor still fully usable (applying happens by clicking the list as today).
- **Interactions**: modal keyboard pattern (Escape/×/backdrop, focus first field on open, focus restore on close); color pickers + synchronized hex inputs; live preview on `input`; copy-prompt buttons unchanged.
- **Edge cases**: renaming a persona in the editor → journey-meta select + journey-list subs update; editing a theme in use → `applyBrandThemeToGrid` reapplies on the next render (call `render()` after save); AI persona answer without `kind` → now imports (D); a themes answer without `kind` → already imports (Change 9); editor lists — blank lines dropped, whitespace trimmed; hex typed as 3/8-digit → valid per `BRAND_THEME_HEX`.
- **Glossary**: `persona editor` (`persona-editor`), `theme editor` (`theme-editor`), `role row` (`theme-role-row`), `live preview` (`theme-preview`), `personas prompt` (`personas-prompt`), `themes prompt` (`themes-prompt`), `theme role labels` (`THEME_ROLE_META`).

**Build instructions for ux-build**
1. **`mkSidebarItem` `onEdit`** (`index.html:2076`): new optional param; when provided AND `onRename` is too, the ✎ button calls `onEdit()` (not `startEdit`); dblclick on the name stays `onRename`. When only `onEdit` is provided (themes): ✎ rendered, no dblclick rename. Existing callers (categories, journeys) unchanged — they pass no `onEdit`.
2. **Persona editor** — `openPersonaEditor(personaId)`: overlay modal on the `openPersonaCard` pattern (`index.html:1438`); panel `persona-card persona-editor`, `width: min(520px, 92vw)`, scrollable, title "New persona" / the persona's name. Fields (imperative DOM, labels via `.pc-label`): Name (input, autofocus), Role (input), Description (textarea rows 3), Quote (textarea rows 2), Goals / Needs / Frustrations (textarea rows 3, placeholder "One per line"). Footer: Cancel (`.btn-ghost .btn-sm`) + Save (`.btn-primary .btn-sm`). Save: `name = trim || \`Persona ${state.personas.length + 1}\``; lists via `normStrList(value.split('\n').join('\n'))` — actually split on newlines: `value.split(/\n/).map(s=>s.trim()).filter(Boolean)`; write to the persona (create mode: `newPersona(name)` pushed first); `save(); renderPersonaList(); renderJourneyMeta(); renderJourneyList();` then close. CSS `.pe-input`/`.pe-area` (inset bg, border, focus accent).
3. **Persona entry points**: `#btn-add-persona` (`index.html:3440`) → `openPersonaEditor(null)`; delete `createPersona()` (`index.html:810`); `renderPersonaList` item gains `onEdit: () => openPersonaEditor(p.id)` (✎ opens the editor, dblclick stays quick rename); persona card footer Edit button → close card + `openPersonaEditor(id)`; card empty-state copy → "No details yet — add them here (Edit) or import personas from JSON (↥)."
4. **Theme editor** — `openThemeEditor(themeId)`: modal on the tp-manager pattern; panel `theme-editor` `width: min(620px, 92vw)`, column flex, scrollable middle. Top: Name input. Body: 13 role rows (`THEME_ROLE_META`: `{ role, label, hint }` for all `THEME_ROLES` entries — e.g. `bg` "Map background", `stageBg` "Stage band", `stageText` "Stage band text", `stepBg` "Step header", `labelBg` "Row-label column", `labelText` "Row-label text", `cellBg` "Cell background", `text` "Step names & cell text", `border` "Strong borders", `borderSoft` "Cell separators", `accent` "Accents & map title", `chipBg` "Pill background", `chipText` "Pill text"`). Row: swatch `<input type="color">` (default value = current hex or the resolved fallback token via `cssVar`), hex text input (`#rrggbb`, 3/6/8-digit OK), clear × (empties the text input), label + hint. Sync: swatch `input` → writes text; text `input` → swatch follows when valid. Preview strip below (or above the footer): a stage band div, a step header div, a label cell + a data cell containing a touchpoint pill ("E-mail") and a text line — recolored on every `input` from `(text valid ? text : cssVar(fallback))`; stageText fallback `--text-2`, others per `ROLE_FALLBACK_TOKEN`, chip roles → `--chip`/`--chip-text`. Footer Cancel + Save. Save: `name = trim || \`Theme ${state.brandThemes.length + 1}\``; `colors = {}` filled from rows whose text matches `BRAND_THEME_HEX`; create → push `{ id: uid(), name, colors }`; edit → replace `name` + `colors` wholesale; `save(); render();` (re-renders list, meta select, themed grid).
5. **Theme entry points**: Themes section header (`index.html:442-448`) gains `+` (`#btn-add-theme`, after ↥/↧) → `openThemeEditor(null)`; `renderThemeList` items gain `onEdit: () => openThemeEditor(t.id)`; empty-state copy → "No themes yet — add one (+) or ↥ import a themes JSON file."
6. **AI prompts — modal refactor** (`openAiPrompt`, `index.html:3101`): replace the two hardcoded tab buttons with `AI_TABS = [{id:'create',label:'Create journey'},{id:'expand',label:'Expand journey'},{id:'personas',label:'Personas'},{id:'themes',label:'Brand themes'}]` rendered in a loop (`.ap-tab`, active class + `aria-pressed`); `renderTab(id)` switches on the id. Create/Expand behavior byte-identical (same constants, same no-journey hint on expand, same Copy JSON only button); Personas/Themes always render their constant + Copy prompt. Per-tab textarea `aria-label`.
7. **`AI_PROMPT_PERSONAS`** (English template literal, sibling of `AI_PROMPT_CREATE`): (1) *Role*: help a UX designer create personas for customer-journey maps; personas are imported into a tool from JSON. (2) *Interview first*: before any JSON ask about 6-10 questions in small rounds of 2-4 — the product/process and its goal; who uses it (segments, roles); each segment's goals and context; what data exists (research, analytics, support tickets, expert knowledge); at any point the user may paste materials — mine them; "generate now" stops the interview. (3) *Persona semantics*: `name` (realistic, not stereotype labels), `role` (job/situation in one line), `description` (2-4 sentence narrative: context, typical day, relationship with the product), `goals` (what they want to achieve), `needs` (what they need to succeed), `frustrations` (pains today), `quote` (first-person one-liner capturing the attitude); 2-4 personas per set, clearly differentiated; assumptions listed before the JSON. (4) *Contract*: `{ "version": 1, "kind": "personas", "app": "customer-journey", "personas": [ { "name": "Anna Kowalska", "role": "Senior accountant", "description": "…", "goals": ["…"], "needs": ["…"], "frustrations": ["…"], "quote": "…" } ] }` — all fields except `name` optional; lists are arrays of short strings; unknown fields ignored; always include `"kind": "personas"`. (5) *Output*: single ```json block, nothing after, valid JSON (no comments/trailing commas).
8. **`AI_PROMPT_THEMES`**: (1) *Role*: help a UX designer build a brand color theme for customer-journey map exports (a poster-style PNG); the theme is imported from JSON. (2) *Input*: the user gives a brand name, website, brand-book hexes, or a mood; if only a brand name is given, use its public brand colors and mark inferred hexes as assumptions; ask at most one short round of questions if nothing usable is given. (3) *Roles table*: the 13 roles with what each paints (the table from instruction 4) — every role optional, missing roles fall back to the tool's neutral gray-blue default. (4) *Color rules*: hex `#rgb`/`#rrggbb` only; keep text-on-background pairs ≥ 4.5:1 WCAG AA (`stageText` on `stageBg`, `labelText` on `labelBg`, `text` on `cellBg`, `chipText` on `chipBg`); borders subtle (`borderSoft` lighter than `border`); light `bg`/`cellBg` recommended for print; `accent` = the brand's primary. (5) *Contract*: `{ "version": 1, "kind": "themes", "app": "customer-journey", "themes": [ { "name": "Acme Corp", "colors": { "bg": "#ffffff", "stageBg": "#e11d48", … all 13 roles … } } ] }` — always include `"kind": "themes"`. (6) *Output*: single ```json block, nothing after.
